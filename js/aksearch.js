...
const searchStructures = async searchText => {
    //use above for online
    const res = await fetch('../bridge-navigator/data/akstructures.json');
    //use below for local 
    // const res = await fetch('data/akstructures.json');
    const structures = await res.json();
    
    //Get matches to current text input
    let matches = structures.filter(bridge => {
        const regex = new RegExp(`${searchText}`, 'gi');
        return bridge.name.match(regex);
    });
